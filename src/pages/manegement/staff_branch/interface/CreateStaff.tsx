...
import { CREATE_EMPLOYEE, DELETE_EMPLOYEE, UPDATE_EMPLOYEE } from "../../../../services";
...
const CreateStaff: React.FC<Props> = ({
  open,
  onClose,
  data,
  editMode,
  ...
  const [createData, { loading: createLoading }] = useMutation(CREATE_EMPLOYEE);
  const [updateData, { loading: updateLoading }] = useMutation(UPDATE_EMPLOYEE);
  const [deleteData, { loading: deleteLoading }] = useMutation(DELETE_EMPLOYEE);
  ...
  const onFinish = async (value: any) => {
    ...
    try {
      if (editMode) {
        ...
        if (result?.data?.updateEmployee?.id) {
        ...
        if (result?.data?.createEmployee?.id) {
      ...
      if (error?.message === "USER_ALREADY_EXIST") {
        return message.warning(`ຊື່ນຳໃຊ້ເຂົ້າລະບົບ ${value.username} ມີແລ້ວໃນຖານຂໍ້ມູນ`);
      } else {
  ...
  return (
    <div>
      <Drawer
        title={editMode ? "ລາຍລະອຽດ" : "ເພີ່ມພະນັກງານໃໝ່"}
        width={400}
