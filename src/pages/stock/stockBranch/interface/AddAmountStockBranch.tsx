...
const AddAmountStockBranch: React.FC<Props> = ({ selectBranch }) => {
  ...
  const [createStockMany, { loading: createLoading }] = useMutation(
    ...
    {
      onCompleted: (data) => {
        message.success(`ອັບເດດສະຕ໋ອກຈຳນວນ ${data?.updateStockMany?.count} ລາຍການສຳເລັດ`);
        refetchStockList();
      },
      onError: (error) => {
            ...
            <Card 
              title={
                <Space>
                  ...
                  <Badge count={stockList?.stocks?.total || 0} />
                </Space>
              }
            >
...
export default AddAmountStockBranch;