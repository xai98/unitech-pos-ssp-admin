...
import { CATEGORIES, GET_STOCK_LOWS } from "../../../../services";
...
const StockLowsBranch: React.FC<Props> = ({ selectBranch }) => {
  ...
  const [loadStockLows, { data: stockList, loading }] = useLazyQuery(GET_STOCK_LOWS, {
    fetchPolicy: "network-only",
  });
  ...
  const dataSource = useMemo(() => {
    return stockList?.stockLows?.data?.map((item: StockItem, index: number) => ({
      key: item.id,
  ...
  return (
    <div style={{
      ...
      ສິນຄ້າຂາຍໜ້າຮ້ານໃກ້ໝົດ
      </Title>

      <Row gutter={[16, 16]} style={{ marginBottom: 24,marginTop:10 }}>
      ...
      <Spin spinning={loading} tip="ກຳລັງໂຫລດຂໍ້ມູນ...">
        <div style={{ marginBottom: 16 }}>
          <Typography.Text strong>
            {stockList?.stockLows?.total || 0} ລາຍການ
          </Typography.Text>
        </div>
        <Table
          columns={columns}
          ...
          pagination={{
            ...
            total: stockList?.stockLows?.total || 0,
...
export default StockLowsBranch;