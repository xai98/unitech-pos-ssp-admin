...
const HistoryStockBranch: React.FC<Props> = ({ selectBranch }) => {
  ...
  const dataSource = useMemo(() => {
    return (
      historyStockData?.historystocks?.data?.map(
        ...
        <Spin spinning={loading} tip="Loading history..." size="large">
          <Space direction="vertical" size="middle" style={{ width: "100%" }}>
            <Typography.Text type="secondary">
              ຈຳນວນ: {historyStockData?.historystocks?.total || 0} ລາຍການ
            </Typography.Text>
            <Table
              columns={columns}
...
export default HistoryStockBranch;
