...
import {
  REPORT_PRODUCT_POPULATION_MONTH,
} from "../../../../services";
...
const ReportYear: React.FC<Props> = ({ selectBranch }) => {
  ...
  const [loadReport, { data: reportProuductPopulation }] = useLazyQuery(
    REPORT_PRODUCT_POPULATION_MONTH,
    {
  ...
  useEffect(() => {
    if (reportProuductPopulation) {
      const monthList =
        reportProuductPopulation?.reportProuductPopulationMonth.map(
          (item: any) => item.month
        ) || [];
      const totalOrders =
        reportProuductPopulation?.reportProuductPopulationMonth.map(
  ...
  const onChange: DatePickerProps["onChange"] = (_, dateString) => {
    if (typeof dateString === "string") {
      ...
      setFilter({
        from_date: selectedDate.startOf("year").format("YYYY-MM-DD"),
        to_date: selectedDate.endOf("year").format("YYYY-MM-DD"),
      });
    } else {
